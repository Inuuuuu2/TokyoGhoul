...
export const DEFAULT_FORMAT_PROMPT = `【⚠️ 输出格式硬性规范 · 红线】

== 红线 == 每次回复都必须包含**非空**的 <maintext>……</maintext>。不管 <thinking> 写得多长多漂亮，缺 maintext 就是空回，玩家界面直接空白 —— 永远先保证 maintext 落地。
== 红线 == 正文（故事内容）只能写在 <maintext> 块里。不要把正文写进 <thinking>。
== 红线 == 不要用 Markdown 代码块（\`\`\`）包裹任何 XML 标签 —— 标签必须裸露。
== 红线 == <thinking> 步骤 1 必须把 [本回合启用的预设条目清单] 的**全部 N 条**都覆盖到（触发详写 + 不触发合并一行），漏一个序号算违规。

输出顺序：<thinking> → <maintext> → <sum> → <vars>（可选）→ <memory>（可选）。

<thinking>
（必填）思维链推理，玩家不可见。**总长建议 ≤ maintext 长度**——这块越长，给 maintext 的 token 越少。

1. 预设巡检（最重要 · 必须覆盖全部 N 个序号）：
   分两段写：

   ▶ **触发**（本回合实际生效，通常 5-15 条）：
   按 "序号. 条目名 → 这回合怎么落地（一句话）" 一行一条。
   - 涉及视角 / 文风 / 调性 / 禁忌 / 抗写 / 抢话 / 复述 / 思考链格式 等的，本回合凡是用到都必须放这里。
   - 引用的序号 / 条目名必须与清单完全一致，不许虚构。

   ▶ **不触发**（其余无关条目，合并成一行汇总）：
   写格式 "不触发：序号 X, Y, A-B, …（共 K 条，原因：与本回合无关）"
   - 用序号区间（如 5-12）压缩，不要逐条展开。
   - "触发 + 不触发" 的序号集合必须凑齐清单的全部 N 条，少一个算违规。

2. 玩家输入：1-2 句，引发了什么状态变化、新事件、情绪转折。
3. 记忆 / 变量：1-2 句，要 update / add 的条目 ID 与要调的状态变量。
4. 写作策略：1-2 句，由步骤 1 "触发" 的条目决定本回合的调性 / 视角 / 节奏。

注意：完整正文 / 对话 / 描写都不写在这里，写在下面的 <maintext> 里。
如果 thinking 已经写了一大堆，立刻收尾跳到 maintext，不要继续展开。
</thinking>
